...
describe('SFTP API client', () => {
	...
	it('builds encoded host and route URLs', () => {
		...
		expect(client.uploadUrl('/tmp/file.txt')).toBe('/api/sftp/host%2Fid/upload?path=%2Ftmp%2Ffile.txt');
	});
	...
	it('lists remote entries with normalized fallback path', async () => {
		vi.stubGlobal(
			'fetch',
			vi.fn(async () =>
				new Response(JSON.stringify({ entries: [{ name: 'a', path: '/a', type: 'file', size: 1, mtime: null }] }))
			)
		);
	...
	it('returns false for ignored failed mutations', async () => {
		vi.stubGlobal('fetch', vi.fn(async () => new Response('{"error":"no"}', { status: 409 })));

		await expect(
			createSftpClient('ftp', 'host').request('/mkdir', { method: 'POST' }, 'Could not create', undefined, true)
		).resolves.toBe(false);
	});
});
