...
- `TERMIXKIT_SSH_TRUST_ON_FIRST_USE`: defaults to enabled when unset, so unknown SSH/SFTP host keys are pinned on first use. Set to `0` or `false` for strict known-host checking.
- `TERMIXKIT_SSH_ALLOW_PRODUCTION_TOFU`: defaults to enabled when unset. Set to `0` or `false` in production only when unknown hosts must be pre-seeded in `TERMIXKIT_SSH_KNOWN_HOSTS_PATH`.
